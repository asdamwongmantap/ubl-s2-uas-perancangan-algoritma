...
func main() {
	...
	fmt.Println("| Program untuk mencetak waktu berpapasan dan jarak badu dengan titik B ")
	fmt.Println("+===========================================+")
	...
	for jarak < 1000 {
		...
		if waktutempuh == 10 {
			...
			waktupapasan = (jarak - selisihjarak) / kecepatankedua
		}
	}

	timeHoursFinalStr, timeMinuteFinalStr, timeSecondFinalStr := convertToTime("08:00:10", int(waktupapasan))
	convertSecondToInt, _ := strconv.Atoi(timeSecondFinalStr)
	jarakBadu = 3 * (convertSecondToInt - int(selisihjarak))
	fmt.Println(fmt.Sprintf("Badu Akan Berpapasan Pada %s:%s:%s ", timeHoursFinalStr, timeMinuteFinalStr, timeSecondFinalStr))
	fmt.Println(fmt.Sprintf("Jarak Badu Dengan Titik B Setelah Berpapasan %d m", jarakBadu))
